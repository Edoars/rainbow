...
public class Debug {
	...
	public void centralMapEval(CentralMap F, int n, byte[] x) {
			...
			for (int j = 0; j < layer.poly.length; j++) {
				...
				for (int r = 0; r < p.quad.length; r++) {
					for (int c = 0; c < p.quad[0].length; c++) {
						if (r <= c && p.quad[r][c] != 0) System.out.print("a^"+GF16.logsTable[p.quad[r][c]]+"*x["+(r+1)+"]*x["+(c+1)+"]+");
					}
				}
				...
				for (int r = 0; r < p.quad.length; r++) {
					for (int c = 0; c < p.quad[0].length; c++) {
						if (p.quad[r][c] != 0) System.out.print("a^"+GF16.logsTable[p.quad[r][c]]+"*x["+(r+1)+"]*x["+(c+vi+1)+"]+");
					}
				}
		...
		if (x[x.length-1] != 0) System.out.println("a^"+GF16.logsTable[x[x.length-1]]+"]) : i in [1.."+(F.o1+F.o2)+"]];");
